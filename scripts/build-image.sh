...
if [ ! -z ${GPT} ]; then
    ...
    parted --script "${disk}" \
    mklabel gpt \
    mkpart primary fat32 16MiB 32MiB \
    mkpart primary fat32 32MiB 544MiB \
    mkpart primary ext4 544MiB 100%
else
    dd if=/dev/zero of="${disk}" count=4096 bs=512
    parted --script $disk \
    mklabel msdos \
...
mkfs.vfat -i "${config_uuid}" -F32 -n config "${disk}${partition_char}1"
mkfs.vfat -i "${boot_uuid}" -F32 -n boot "${disk}${partition_char}2"
dd if=/dev/zero of="${disk}${partition_char}3" bs=1KB count=10 > /dev/null
mkfs.ext4 -U "${root_uuid}" -L rootfs "${disk}${partition_char}3"

# Mount partitions
mkdir -p ${mount_point}/rootfs
mount "${disk}${partition_char}3" ${mount_point}/rootfs

# Copy the rootfs to root partition
echo -e "Decompressing $(basename "${rootfs}")\n"
tar -xpJf "${rootfs}" -C ${mount_point}/rootfs
...
if [ ! -z ${GPT} ]; then
cat > ${mount_point}/rootfs/etc/fstab << EOF
# <file system>     <mount point>  <type>  <options>   <dump>  <fsck>
UUID=${root_uuid,,} /              ext4    defaults    0       1
UUID=${config_uuid,,}	/config	vfat	defaults,x-systemd.automount	0	2
UUID=${boot_uuid}	/boot/efi	vfat	defaults,x-systemd.automount	0	2
EOF

# Uboot env
mkdir ${mount_point}/rootfs/extlinux/
cat > ${mount_point}/rootfs/extlinux/extlinux.txt << EOF
...
label l0
	...
	append root=UUID=${root_uuid} console=ttyFIQ0,1500000n8 quiet splash loglevel=4 rw earlycon consoleblank=0 console=tty1 coherent_pool=2M irqchip.gicv3_pseudo_nmi=0 cgroup_enable=cpuset cgroup_memory=1 cgroup_enable=memory swapaccount=1
...
if [[ "${BOARD}" == radxa-zero ]]; then
    ...
    idbloader=${mount_point}/rootfs/boot/idbloader.img
    uboot=${mount_point}/rootfs/boot/u-boot.itb
...
# [ -z "${img##*server*}" ] && cp ../rootfs/boot/firmware/{meta-data,user-data,network-config} ${mount_point}/system-boot
