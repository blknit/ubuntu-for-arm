...
kernel_dir=../packages/linux-image-mangopi-h616
uboot_dir=../packages/uboot-mangopi-h616

export kernel_deb=$(cd ${kernel_dir}/debian; KDEB_PKGVERSION=$(dpkg-parsechangelog -SVersion -l changelog); source upstream; arch=$(cat arch); echo linux-image-${VERSION}-mangopi-h616_${KDEB_PKGVERSION}_${arch}.deb)
export uboot_deb=$(cd ${uboot_dir}/debian; KDEB_PKGVERSION=$(dpkg-parsechangelog -SVersion -l changelog); arch=$(cat arch); echo uboot-mangopi-h616_${KDEB_PKGVERSION}_${arch}.deb)
