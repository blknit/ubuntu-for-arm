...
kernel_dir=../packages/linux-image-radxa-zero3
uboot_dir=../packages/uboot-radxa-zero3

export kernel_deb=$(cd ${kernel_dir}/debian; KDEB_PKGVERSION=$(dpkg-parsechangelog -SVersion -l changelog); source upstream; arch=$(cat arch); echo linux-image-${VERSION}-radxa-zero3_${KDEB_PKGVERSION}_${arch}.deb)
export uboot_deb=$(cd ${uboot_dir}/debian; KDEB_PKGVERSION=$(dpkg-parsechangelog -SVersion -l changelog); arch=$(cat arch); echo uboot-radxa-zero3_${KDEB_PKGVERSION}_${arch}.deb)
